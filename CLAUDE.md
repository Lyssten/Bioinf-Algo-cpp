...
└── lectures/
    ...
    ├── lecture_04/               # Графовые алгоритмы (6 задач)
    └── lecture_05/               # Графовые алгоритмы, продолжение (7 задач)
```
...
| Директория | Rosalind | Суть |
|-----------|----------|------|
| ba3a-string_composition | BA3A | k-мер композиция строки |
| ba3b-genome_path_string | BA3B | Реконструкция строки из пути генома |
| ba3c-overlap_graph | BA3C | Граф перекрытий k-меров |
| ba3d-de_bruijn_string | BA3D | Граф де Брёйна строки |
| ba3e-de_bruijn_kmers | BA3E | Граф де Брёйна k-меров |
| ba3f-eulerian_cycle | BA3F | Эйлеров цикл в графе |

### Lecture 05: Графовые алгоритмы (продолжение)
| Директория | Rosalind | Суть |
|-----------|----------|------|
| ba3g-eulerian_path | BA3G | Эйлеров путь в графе |
| ba3h-reconstruct_from_kmers | BA3H | Сборка строки из k-мер композиции |
| ba3i-k_universal_circular | BA3I | k-универсальная циклическая двоичная строка |
| ba3j-reconstruct_from_pairs | BA3J | Сборка строки из парных чтений (k,d)-меров |
| ba3k-generate_contigs | BA3K | Генерация контигов из коллекции ридов |
| ba3l-gapped_genome_path | BA3L | Строка из пути с пропусками |
| ba3m-maximal_nonbranching_paths | BA3M | Максимальные неветвящиеся пути в графе |
...
2. **Теория** — математика, ключевые свойства, визуальные примеры
3. **Применение** — конкретные примеры в биологии (организмы, инструменты, числа) и в других областях
4. **Алгоритм** — идея (почему работает) + псевдокод
5. **Реализация на C++** — код с пронумерованными комментариями
6. **Разбор кода** — подробные пояснения C++ синтаксиса и алгоритмических решений
7. **Запуск** — примеры CLI
8. **Анализ сложности** — время и память с обоснованием, **обязательно** со ссылкой на конкретный фрагмент кода (узкое место)
