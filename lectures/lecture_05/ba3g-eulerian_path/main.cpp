...
std::string resolveOutputPath(const CliOptions& options) {
    ...
    return "eulerian_path_output.txt";
}

std::string formatResult(const std::vector<int>& path) {
    std::string result;
...
int main(int argc, char* argv[]) {
    try {
        ...
        auto path = eulerianPath(input.graph);
        std::string result = formatResult(path);
        ...
        std::cerr << "Usage:\n";
        std::cerr << "  ./ba3g-eulerian_path [--input input.txt] [--output output.txt]\n";
        std::cerr << "  ./ba3g-eulerian_path input.txt\n";
        return 1;
    }
}
