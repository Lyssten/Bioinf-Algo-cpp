...
std::vector<std::string> greedyMotifSearch(const std::vector<std::string>& dna, size_t k, size_t t) {
...
GreedyMotifSearchInput sampleInput() {
    return {
        3,
        5,
        {
...
GreedyMotifSearchInput parseInputFile(const std::string& path) {
    std::ifstream input(path);
    if (!input)
        throw std::runtime_error("Cannot open input file: " + path);

    GreedyMotifSearchInput data{};
...
GreedyMotifSearchInput parsePositionalArgs(const std::vector<std::string>& positional) {
...
void validateInput(const GreedyMotifSearchInput& data) {
    if (data.k == 0)
...
std::string resolveOutputPath(const CliOptions& options) {
    ...
    return "greedy_motif_search_output.txt";
}
...
int main(int argc, char* argv[]) {
    try {
        ...
        GreedyMotifSearchInput input = sampleInput();
        ...
        auto motifs = greedyMotifSearch(input.dna, input.k, input.t);
        ...
        std::cerr << "Usage:\n";
        std::cerr << "  ./greedy_motif_search [--input input.txt] [--output output.txt]\n";
        std::cerr << "  ./greedy_motif_search [--output output.txt] k t dna1 dna2 ... dna_t\n";
        std::cerr << "  ./greedy_motif_search input.txt [--output output.txt]\n";
        return 1;
    }
}
