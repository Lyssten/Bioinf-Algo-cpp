...
| Имя | Что это |
|-----|---------|
| `v` | первая входная строка; соответствует **строкам** матрицы |
| `w` | вторая входная строка; соответствует **столбцам** матрицы |
| `n` | длина v |
| `m` | длина w |
| `sigma` | штраф за **открытие** гэпа — первый символ пропуска (= 11) |
| `epsilon` | штраф за **продление** гэпа — каждый последующий символ пропуска (= 1) |
| `lower[i][j]` | лучший счёт, если последнее действие — гэп в v (символ `w[j-1]` выровнен против `-`) |
| `upper[i][j]` | лучший счёт, если последнее действие — гэп в w (символ `v[i-1]` выровнен против `-`) |
| `middle[i][j]` | лучший счёт, если последнее действие — совпадение/замена (диагональный шаг) |
| `backL[i][j]` | направление перехода в `lower[i][j]`: из lower (продление) или из middle (открытие) |
| `backU[i][j]` | направление перехода в `upper[i][j]` |
| `backM[i][j]` | направление перехода в `middle[i][j]`: из lower, upper или диагональ |
| `curState` | текущая матрица в процессе backtrack: 0 = lower, 1 = upper, 2 = middle |
| `startState` | матрица, в которой достигается максимум в ячейке (n, m) |
| `NEG_INF` | «минус бесконечность» = `INT_MIN/2`; начальное значение для недостижимых ячеек |
| `alignV` | строка выравнивания для v |
| `alignW` | строка выравнивания для w |
| **DP** | динамическое программирование; здесь — три связанные таблицы `lower`, `upper`, `middle` |
| **backtrack** | восстановление выравнивания: идём по матрицам backL/backU/backM в обратном порядке от (n, m), переключаясь между тремя матрицами |
| **log-odds score** | значение PAM/BLOSUM: логарифм отношения p(наблюдаемая замена) / p(случайная замена); положительный — замена чаще случайной, отрицательный — реже |
