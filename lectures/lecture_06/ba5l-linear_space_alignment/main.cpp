...
// Compute DP column at index targetCol (linear gap penalty sigma)
...
static MiddleEdge findMiddleEdge(const std::string& v, const std::string& w, int sigma) {
    int n = static_cast<int>(v.size());
    ...
    for (int i = 0; i <= n; ++i) {
        if (mid + 1 <= m) {
            int t = fromSrc[i] - sigma + toSink(i);
            if (t > bestScore) { bestScore = t; bestI = i; bestDiag = false; }
        }
        if (i < n && mid < m) {
            int t = fromSrc[i] + blosum(v[i], w[mid]) + toSink(i + 1);
            if (t > bestScore) { bestScore = t; bestI = i; bestDiag = true; }
        }
    }

    if (bestDiag) return {bestI, mid, bestI + 1, mid + 1, true};
    else          return {bestI, mid, bestI,     mid + 1, false};
}

// Hirschberg recursive alignment; returns (alignedV, alignedW)
    ...
    const std::string& v, const std::string& w, int sigma)
{
    ...
    if (n == 0) return {std::string(m, '-'), w};
    if (m == 0) return {v, std::string(n, '-')};
    ...
    if (e.diagonal) {
        ...
        // Horizontal edge: gap in v
        std::string vL = v.substr(0, r1);
        std::string wL = w.substr(0, e.c1);
        std::string vR = v.substr(r1);
        std::string wR = w.substr(e.c2);
        auto [A1, B1] = hirschberg(vL, wL, sigma);
        auto [A2, B2] = hirschberg(vR, wR, sigma);
...
// Compute alignment score from aligned strings
...
struct InputData { std::string v, w; };
struct CliOptions {
    std::string inputPath, outputPath;
    std::vector<std::string> positional;
};

InputData sampleInput() { return {"PLEASANTLY", "MEANLY"}; }
...
std::string resolveOutputPath(const CliOptions& options) {
    ...
    return "ba5l-linear_space_alignment_output.txt";
}
...
int main(int argc, char* argv[]) {
    try {
        ...
        validateInput(input);
        auto [aV, aW] = hirschberg(input.v, input.w, 5);
        int sc = alignScore(aV, aW, 5);

        Alignment aln{sc, aV, aW};
        ...
        std::cerr << "Usage: ./ba5l-linear_space_alignment [--input in.txt] [--output out.txt]\n";
        return 1;
    }
}
